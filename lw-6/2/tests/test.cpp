...
std::string DrawShapeWithClassAdapterFromStart(const std::vector<mgl::Point>& points)
{
	std::stringstream strm;
	app::ModernGraphicsClassAdapter adapter(strm);

	DrawWithAdapterFromStart(adapter, adapter, points);
...
TEST(ClassAdapterTest, DrawWithClassAdapterFromStart)
{
	auto triangle = {
		mgl::Point(10, 15),
		mgl::Point(100, 200),
	...
	const auto withAdapterShape = DrawShapeWithClassAdapterFromStart(triangle);
