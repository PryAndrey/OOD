...
int main()
{
	...
	cout << endl;
	{
		auto tea = make_unique<CTea>(TeaType::Rainbow)
			<< MakeCondiment<CChocolateSlice>(7)
			<< MakeCondiment<CCream>()
			<< MakeCondiment<CLiqueur>(LiqueurType::Chocolate);
		auto milkshake = make_unique<CMilkshake>(PortionSize::Large);
		auto latte = make_unique<CLatte>(PortionMultiplier::Double);
		auto cappuccino = make_unique<CCappuccino>(PortionMultiplier::Double);
